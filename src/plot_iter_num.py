...
def main():
    ...
    # shilo データの読み込み
    shilo_2_shibuya = pd.read_csv("rmsdh_result/fast_rmsdh_hingek_cnt_2_postpro_loop.csv")
    shilo_3_shibuya = pd.read_csv("rmsdh_result/fast_rmsdh_hingek_cnt_3_postpro_loop.csv")
    shilo_4_shibuya = pd.read_csv("rmsdh_result/fast_rmsdh_hingek_cnt_4_postpro_loop.csv")
    ...
    for i, (rilo_list, shilo_list, row_title) in enumerate(zip(rilo_datasets, shilo_datasets, titles)):
        for j, (rilo_df, shilo_df, col_title) in enumerate(zip(rilo_list, shilo_list, col_titles)):
            iter_num_cols = [col for col in iter_num_cols_list if col in rilo_df.columns]
            rilo_counts = rilo_df[iter_num_cols].apply(pd.value_counts).fillna(0).sum(axis=1).reset_index()
            ...
            axes[i, j].bar(rilo_counts["Value"], rilo_counts["Percentage"], label="R + ILO", alpha=0.7)
            axes[i, j].bar(shilo_counts["Value"], shilo_counts["Percentage"], label="SH + ILO", alpha=0.5)
