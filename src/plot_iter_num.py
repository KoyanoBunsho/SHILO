...
def main():
    ...
    for i, (rilo_list, shilo_list, row_title) in enumerate(
        ...
        for j, (rilo_df, shilo_df, col_title) in enumerate(
            ...
            # 平均値の計算（プロットはせず、標準出力へ出力）
            if total_rilo > 0:
                rilo_avg = (
                    rilo_counts["Value"] * rilo_counts["Count"]
                ).sum() / total_rilo
            else:
                rilo_avg = None
            ...
            if total_shilo > 0:
                shilo_avg = (
                    shilo_counts["Value"] * shilo_counts["Count"]
                ).sum() / total_shilo
            else:
                shilo_avg = None

            print(
                f"{row_title} - {col_title}: R+ILO Avg = {rilo_avg}, SH+ILO Avg = {shilo_avg}"
            )
