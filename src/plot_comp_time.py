...
def main():
    ...
    for k in range(2, k_max):
        ...
        rlo_dyndom_dict[k] = pd.read_csv(f"rmsdh_result/ablation_study_dyndom_{k}.csv.gz")
        ...
        sh_lo_par_dict[k] = pd.read_csv(f"rmsdh_result/fast_rmsdhk_more_data_{k}_pospro.csv")
        sh_lo_dyndom_dict[k] = pd.read_csv(f"rmsdh_result/fast_rmsdh_hingek_cnt_dyndom_{k}_postpro.csv")

        shilo_par_dict[k] = pd.read_csv(f"rmsdh_result/fast_rmsdhk_more_data_{k}_pospro_loop.csv")
    ...
    for k in range(2, k_max):
        comp_time_par["R+LO"].append(rlo_par_dict[k][comp_time_cols_list].sum().mean() / 1000)
        comp_time_par["R+ILO"].append(rilo_par_dict[k][comp_time_cols_list].sum().mean() / 1000)
        ...
        comp_time_dyndom["R+LO"].append(rlo_dyndom_dict[k][comp_time_cols_list].sum().mean() / 1000)
        comp_time_dyndom["R+ILO"].append(rilo_dyndom_dict[k][comp_time_cols_list].sum().mean() / 1000)
    ...
    axes[1].set_xlabel("k")
    axes[1].set_xticks(range(2, k_max))
