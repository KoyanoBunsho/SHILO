...
def process_row(row):
    ...
    hinge_info = row["hinge_file_path"]
    return mapped_hinge_positions_str, hinge_count, hinge_info
...
def main():
    ...
    results = Parallel(n_jobs=-1)(
        delayed(process_row)(row) for row in tqdm(records, total=len(records))
    )
    hinge_positions_list, hinge_count_list, hinge_info_list = zip(*results)
