...
int main(int argc, char **argv) {
  ...
  myfile.open("rmsdh_result/fast_rmsdhk_dyndom_" + std::to_string(hinge_num) +
              "_postpro_loop.csv");
  myfile << "p_pdb_id"
         << ","
         << "q_pdb_id"
  ...
  for (int i = 0; i < (int)pdb_pair_vec.size(); i++) {
    ...
    AblationResult rmsdh_hinge_cnt_result = rmsdh_hinge.CalcFastRMSDhKLoop();
    ...
    myfile << rmsdh_result << "," << hinge_num << "," << hinge_cnt << ","
           << hinge_index << "," << rmsdh_hinge_cnt_result.iter_num << ","
